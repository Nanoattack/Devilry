...
public class PathParticleOptions implements ParticleOptions {
    ...
    public static final Codec<PathParticleOptions> CODEC = RecordCodecBuilder.create((instance) ->
            instance.group(StringRepresentable.fromEnum(Interpolation::values).fieldOf("interpolation").forGetter(options ->
                    ...
                    options.lifeTime
            )).apply(instance, PathParticleOptions::new));
    public static final ParticleOptions.Deserializer<PathParticleOptions> DESERIALIZER = new ParticleOptions.Deserializer<>() {
        public PathParticleOptions fromCommand(ParticleType<PathParticleOptions> particleType, StringReader reader) throws CommandSyntaxException {
            ...
            return new PathParticleOptions(interpolation, nodes, speed, frameSpeed, end, spriteEnd, lifeTime);
        }

        public PathParticleOptions fromNetwork(ParticleType<PathParticleOptions> type, FriendlyByteBuf buf) {
            return new PathParticleOptions(buf.readEnum(Interpolation.class), buf.readList(FriendlyByteBuf::readVector3f), buf.readFloat(), buf.readFloat(), buf.readEnum(End.class), buf.readEnum(End.class), buf.readInt());
        }
    };

    public PathParticleOptions(Interpolation interpolation, List<Vector3f> nodes, float speed, float frameSpeed, End end, End spriteEnd, int lifeTime) {
